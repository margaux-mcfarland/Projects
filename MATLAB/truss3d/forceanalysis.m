...
%add unit vectors of weight at each joint to A mat
% weightUnit = zeros(3*numjoints,numjoints);
% for i = 1:numjoints
%     weightUnit(3*i,i) = -1;
% end
% Amat = [Amat weightUnit];
% %weight at joints
% weight  = calcWeight(numbars,connectivity,joints,numjoints);
% build load vector
for i=1:numloads
...
for i = 3:3:length(bvec)
       
        bvec(i) = bvec(i) + weight(i/3);
